...
| `HERMES_BRANCH` / `HERMES_REF` | Branch the installer clones (default `main`) and the commit it pins to (default: a v0.20.4-era `main` commit, what the live device runs). Set `HERMES_REF` empty to track the branch HEAD |
...
- **Browser engine is the apt `chromium`.** `agent-browser` keeps its own browser cache under `/root/.agent-browser/browsers`, but that cache starts empty and it falls back to system Chrome, so the apt package is what actually drives the browser (`agent-browser doctor` says "Falling back to system Chrome ... /usr/bin/chromium"). Google ships no Chrome build for arm64 Linux, so Chromium is the Chromium-family browser on a Pi. The `/browser connect` CDP path is separate and still needs a browser launched with `--remote-debugging-port` and a dedicated `--user-data-dir`, because Chromium 136+ silently refuses the debug port on the default profile.
- **`browser.backend` must stay the quoted string `"off"`.** YAML 1.1 parses a bare `off` as boolean `false`, which is a different value than the string Hermes wants (same trap as `approvals.mode`). `"off"` selects the built-in `browser_*` tools. Leaving the key unset is not equivalent: `uvx` is on root's PATH, so Hermes would pick Browser Use mode instead, a downloaded harness that runs model-written Python and needs the terminal toolset. Presync re-asserts the quoted form.
- **Web search needs no key.** `ddgs` is the only backend Hermes gates on a Python package being importable rather than an API key, so `web.search_backend: ddgs` gives a keyless device real `web_search`. `web_extract` is not covered: ddgs is search-only, and clean page extraction still wants a Firecrawl/Tavily/Exa key.
